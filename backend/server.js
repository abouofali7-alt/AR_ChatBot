...
app.post('/api/project/build', auth.requireAuth, async (req, res) => {
  ...
  try {
    ...
    const config = loadJSON('config.json');
    const projectPrompt = `You are a SENIOR SOFTWARE ARCHITECT. Generate a COMPLETE, PRODUCTION-READY project as a SINGLE self-contained file.\n\nProject Description: ${description}\nLanguage: ${language || 'auto'}\nFramework: ${framework || 'auto'}\n\nOUTPUT FORMAT — Output EXACTLY this JSON structure inside a single markdown code block tagged \`\`\`json:\n{\n  "name": "project-name",\n  "description": "Brief project description",\n  "files": [\n    {\n      "path": "relative/file/path.ext",\n      "content": "FULL complete file content with proper indentation and line breaks",\n      "explanation": "Detailed explanation of what this file does"\n    }\n  ],\n  "setup": [\n    "Step 1: ...",\n    "Step 2: ..."\n  ],\n  "steps": [\n    {\n      "number": 1,\n      "title": "Step title",\n      "description": "Detailed explanation",\n      "files": ["file1.ext"]\n    }\n  ]\n}\n\nSTRICT RULES:\n1. EVERY file must be 100% COMPLETE — no placeholders, no "// add more here", no "// TODO"\n2. Every function must be FULLY implemented with real logic\n3. For web projects: ALWAYS produce a SINGLE index.html file with ALL CSS and JS embedded inline inside <style> and <script> tags. Do NOT reference external app.js or style.css files.\n4. The single index.html must be a FULLY WORKING application that runs by just opening it in a browser\n5. The index.html must include: complete HTML structure, embedded CSS styling, embedded JavaScript with ALL functions implemented\n6. All file content strings must use \\n for newlines (not actual newlines), and escape quotes properly\n7. Include README.md as a second file with setup instructions\n8. Program must work immediately when opened in browser\n9. No text outside the JSON code block`;
